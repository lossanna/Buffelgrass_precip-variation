...
# Change from initial submitted version:
#   1. Corrected a couple of instances of incorrect Plot, as well as one instance of Plant_ID 
#     (see 11.R).
#   2. "Flat" aspect plot included, as aspect is no longer a predictor.
...
culm.change <- read_csv("data/publish2.3/culm-data.csv")
plot.change <- read_csv("data/publish2.3/plot-data.csv")
survival <- read_csv("data/publish2.3/survival-data.csv")
prism.dat <- read_csv("data/publish2.3/site-info.csv")
...
length(unique(dat$Plant_ID)) # 782
...
summary(filter(dat, Site %in% c("KinneyHill", "ApachePeak"))$PlotSlope) # 4 to 28
...
save(actual.summary.year2,
     file = "RData/publish2.3_actual-summary.RData")
