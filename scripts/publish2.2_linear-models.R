...
bgden <- lmer(Change_BGDensity ~ Prev_year_precip_scaled + 
               ...
               data = plot.change)
summary(bgden)
r2(bgden) # marginal: 0.422; conditional: 0.468
...
bgcov <- lmer(Change_BGCover ~ Prev_year_precip_scaled + 
               ...
               data = plot.change)
summary(bgcov)
r2(bgcov) # marginal: 0.261; conditional: 0.285
...
survival <- glmer(cbind(seedlings_surviving,
                   ...
                   data = dat.survival)
summary(survival)
r2(survival) # marginal: 0.493; conditional: 0.966
...
save(culm.change, plot.change, dat.survival, 
     ...
     file = "RData/publish2.2_data-and-models.RData")


save.image("RData/publish2.2_linear-models.RData")
